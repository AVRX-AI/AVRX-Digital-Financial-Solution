...
function createSmtpTransporter(): nodemailer.Transporter | null {
  ...
  if (!pass) {
    console.warn('[AVRX SMTP NOTICE] process.env.SMTP_PASS is not configured. Email running in safe mode.');
    return null;
  }
  ...
  if (transporter) {
    try {
      ...
      try {
        ...
        console.error(`[AVRX SMTP ERROR] Failed sending to admin ${adminEmail}:`, err?.message || 'Admin email dispatch error');
      ...
      try {
        ...
        console.error(`[AVRX SMTP ERROR] Failed sending auto-reply to client ${lead.email}:`, err?.message || 'Customer email dispatch error');
        if (!lastError) lastError = err?.message || 'Customer email dispatch failed';
      }

    } catch (err: any) {
      console.error(`[AVRX SMTP CRITICAL ERROR] SMTP transporter error:`, err?.message || 'Transport error');
    ...
    clientEmailSent = true;
    adminEmailSent = true;
    console.warn(`[AVRX SMTP SIMULATION] SMTP credentials not configured yet. Lead ${lead.id} recorded safely.`);
  }

  // Save to lead record store
