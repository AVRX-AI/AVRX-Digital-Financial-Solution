...
export default function ContactPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    // FormSubmit autoresponder does NOT work with AJAX submissions.
    // The form therefore uses a normal POST submission to a hidden iframe.
    // This keeps the user on the same page while allowing _autoresponse to work.
            ...
            <div className="lg:col-span-7">
              <div className="glass-card p-8 sm:p-12 rounded-3xl border border-white/15 bg-[#0C0F1D]/90 shadow-2xl space-y-8">
                ...
                {formSubmitted ? (
                  ...
                  <iframe name="formsubmit_iframe" title="Form submission" style={{ display: 'none' }} />
                    <form
                      onSubmit={handleSubmit}
                      action="https://formsubmit.co/support@avrx.in"
                      method="POST"
                      target="formsubmit_iframe"
                      className="space-y-4"
                    >
                      <input type="hidden" name="_subject" value={`New AVRX Website Enquiry - ${formData.service}`} />
                      <input type="hidden" name="_replyto" value={formData.email} />
                      <input
                        type="hidden"
                        name="_autoresponse"
                        value={`Hello ${formData.name || 'there'},

Thank you for contacting AVRX Digital & Financial Solution.

We have successfully received your enquiry regarding ${formData.service}.
